...
int Traffic_Info_Save::get_pps_info(int if_cnt)
{
    ...
    {   
        ...
        // pps 계산
        if(pps_snmp_operate(status_int, loop_cnt, 0, rTime_d, "11", res_pdu_ptr) == -1)
        {
            std::cout << "snmp err 0\n";
        }
        ...
        // ppd 계산
        if(pps_snmp_operate(status_int, loop_cnt, 1, rTime_d, "17", res_pdu_ptr)== -1)
        {
            std::cout << "snmp err 1\n";
        }
        ...
        // 인터페이스별 pps 계산
        if(pps_snmp_operate(status_int, loop_cnt, 2, rTime_d, "12", res_pdu_ptr) == -1)
        {
            std::cout << "snmp err 2\n";
        }
        ...
        // 인터페이스 별 pps 계산
        if(pps_snmp_operate(status_int, loop_cnt, 3, rTime_d, "18", res_pdu_ptr) == -1)
        {
            std::cout << "snmp err 3\n";
        }
...
// pps 처리
int Traffic_Info_Save::pps_snmp_operate(int status_int,  int loop_cnt, int index_num,  double runtime, std::string mib_check_str, struct snmp_pdu *res_pdu_ptr)
{
    double pps_d;
    long pps_l;
