...
{ 
    public: 
        explicit Interface_Info_Save(const char* ip, const char* community); // 생성자 snmp pdu, session 초기화
        ~Interface_Info_Save(); 
        int ifInfo_save_db(std::map<std::string, std::string> if_port_map); // 생성된 map을 활용해 인터페이스 정보 저장
        int state_map_renew(int if_cnt); // 인터페이스 상태정보 맵 갱신
        int if_name_renew(int if_cnt);
    ...
    private : 
        // 인터페이스 - 이름 맵
        std::map<std::string, std::string> insterface_name_map;
        // snmp 관련 변수 
...
{
    public : 
        explicit Interface_Map_Info(const char* ip, const char* community); // 생성자 snmp pdu, session 초기화
        ~Interface_Map_Info();
        void map_init();
